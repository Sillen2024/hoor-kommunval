...
- [x] **30c. P3 KLAR 2026-08-17** (commits `02344fe`, `2138526`, `36ecf26` — en per delpunkt):
  1. `trygghet-migration` delad i `migration` + `hardare-straff` — sajtens sista dubbelfråga, 12 frågor totalt. Källorna följer rätt led (SD:s Migration- respektive Trygghet-ståndpunkt), `kompetens: "nationell"` på båda, `metod_not` säger tre nationella frågor. Antalet läses ur datan och visar 12 överallt i byggd HTML (startsidan, `/valkompass/`, `/om-urvalet/`, "Fråga 1 av 12"); gruppräkningen i `Valkompass.astro` är dynamisk. `og-valkompass-v1.png` omgenererad ("12 frågor om Höör", samma motiv) — **Sharing Debugger för `/valkompass/` efter deploy.**
  2. Sidfotens löptext länkar inte längre AI-experimentet, Om urvalet och Rätta en uppgift (menyn strax under har dem); texten står kvar, per-sida-mailton är kvar. Varje mål länkas nu exakt en gång i sidfoten.
  3. **Upptäckt:** "Löftet och pengarna" (§2D) fanns redan i release-committen och renderas på S/V/MP (senaste nämndramar i mnkr mot styrets budget) — granskning 2:s P3.13 var en kvarhängd punkt från granskning 1. Det som saknades var styrets partier: nu en villkorad paragraf på M/L/C/KD med skattesatsens väg 21,75 → 20,95 ur `budget.json` (riktningsordet räknas ur talen). SD/MED får medvetet ingen sektion — att de saknar egna budgetförslag är inte källbelagt i datan; vill Jonas ha den raden krävs först ett belagt frånvaro-konstaterande (jfr M3-mönstret "kontrollerad frånvaro").
...
- [x] **31b. GDPR-genomgång och repohygien** (`b7eb181`, `0a51d7e`, `4ed8042`).
  ...
  - Samma bedömning tillämpad på S (2026-08-19): råtranskriptionen av förskole-videon ute ur det publika repot, mappen gitignorad, lokal kopia kvar. Källhänvisningen i `S_KALLOR_2026.md` uppdaterad; ståndpunktskortet i `partier.json` länkar som förut till det publika inlägget.
